...
class SubMaterialsSeeder extends Seeder
{
    public function run(): void
    {
        $materialTopicMap = [
            1 => [ // Pengantar Konsep Dasar OOP
                [
                    ...
                    'jenis'   => ContentCategory::TEORI->value,
                    'style'   => LearningStyle::TEXTUAL->value,
                ...
                [
                    ...
                    'jenis'   => ContentCategory::SINTAKS->value,
                    'style'   => LearningStyle::VISUAL->value,
                ...
                [
                    ...
                    'jenis'   => ContentCategory::MIXED->value,
                    'style'   => LearningStyle::VISUAL->value,
        ...
        foreach ($materialTopicMap as $moduleId => $subTopics) {
            $material = Material::where('module_id', (string) $moduleId)->first();

            if (! $material) {
                continue;
            }

            // Delete questions for existing submaterials first to avoid orphan data
            $existingSubMaterials = $material->subMaterials()->get();
            foreach ($existingSubMaterials as $sub) {
                $sub->questions()->delete();
            }
            ...
            foreach ($subTopics as $index => $topic) {
                SubMaterial::create([
                    ...
                    'jenis_konten'   => $topic['jenis'] ?? ContentCategory::TEORI->value,
                    'learning_style' => $topic['style'] ?? LearningStyle::TEXTUAL->value,
                    'order'          => $index + 1,
                ]);
            }
        }
