        ...
        <x-ui.card class="border-slate-100 shadow-2xl">
            <form action="{{ route('admin.materials.submaterials.update', [$material->id, $submaterial->id]) }}" method="POST" class="space-y-8">
                ...
                <x-forms.form-group label="Isi Materi" name="content" required>
                    <textarea name="content" rows="12" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-blue-100 focus:border-blue-600 transition-all outline-none" placeholder="Tuliskan isi materi atau link video/gambar di sini..." required>{{ old('content', $submaterial->content) }}</textarea>
                </x-forms.form-group>
