<x-layouts.app title="Tambah Sub-materi" theme="admin">
    <div class="max-w-4xl mx-auto space-y-12">
        ...
        <x-ui.card class="border-slate-100 shadow-2xl">
            <form action="{{ route('admin.materials.submaterials.store', $material->id) }}" method="POST" class="space-y-8">
                @csrf
                <input type="hidden" name="material_id" value="{{ $material->id }}">

                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <x-forms.form-group label="Judul Sub-materi" name="title" required>
                        <x-ui.input name="title" placeholder="Contoh: Pengenalan Class & Object" required value="{{ old('title') }}" />
                    </x-forms.form-group>

                    <x-forms.form-group label="Urutan Tampil" name="order" required>
                        <x-ui.input type="number" name="order" placeholder="1" required value="{{ old('order', ($material->subMaterials->max('order') ?? 0) + 1) }}" />
                    </x-forms.form-group>
                </div>
                ...
                <x-forms.form-group label="Jenis Konten Utama" name="jenis_konten" required>
                    <select name="jenis_konten" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-blue-100 focus:border-blue-600 transition-all outline-none">
                        <option value="teori" {{ old('jenis_konten') == 'teori' ? 'selected' : '' }}>Teori (Konsep Dasar)</option>
                        <option value="sintaks" {{ old('jenis_konten') == 'sintaks' ? 'selected' : '' }}>Sintaks (Kode Program)</option>
                        <option value="mixed" {{ old('jenis_konten') == 'mixed' ? 'selected' : '' }}>Mixed (Teori & Kode)</option>
                    </select>
                </x-forms.form-group>
                ...
                <div class="pt-6 border-t border-slate-100 flex justify-end">
                    <x-ui.button type="submit" variant="primary" size="lg" icon="fas fa-save">Simpan Sub-materi</x-ui.button>
                </div>
            </form>
        </x-ui.card>
    </div>
