    ...
    <x-slot:scripts>
        ...
        function initializeQuestionForm() {
            ...
            function showAllQuestionsCompleted() {
                ...
                // Tampilkan pesan semua soal telah terjawab - Using raw HTML to match card style roughly
                const questionContainer = document.getElementById('questionContainer');
                if (questionContainer) {
                    questionContainer.innerHTML = `
                        <div class="card card-component shadow-sm text-center py-5">
                            <div class="card-body">
                                <div class="mb-4">
                                    <i class="fas fa-check-circle text-success" style="font-size: 5rem;"></i>
                                </div>
        ...
        function submitAnswer() {
            ...
            if (!selectedOption) {
                // Assuming showAlert is global or I need to define/use existing alert mechanism
            ...
            .then(data => {
                ...
                .then(response => response.json())
                .then(result => {
                    showFeedback(result); // Using updated showFeedback signature derived from original code context where result encapsulates needed info, or adapt if needed
            ...
            .catch(error => {
                console.error('Error:', error);
                alert('Terjadi kesalahan saat mengambil data percobaan');
                if (submitButton) {
                    submitButton.disabled = false;
                    submitButton.innerHTML = '<i class="fas fa-check-circle me-2"></i>Periksa Jawaban';
                }
        ...
        // Event handler submit for jQuery (legacy support from original file)
        // Kept for compatibility if jQuery usage persists in partials
        if (typeof $ !== 'undefined') {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.status === 'success') {
                                // Show success feedback - adapted to match logic
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Only if current question exists
            @if($currentQuestion)
