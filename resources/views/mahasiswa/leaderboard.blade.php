                            ...
                            @if(isset($leaderboardData[1]) && $leaderboardData[1]->total_correct_questions > 0)
                                <div class="text-center mb-8 group">
                                    <div class="relative mb-6">
                                        ...
                                        <div class="absolute -bottom-2 -right-2 w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-lg border-2 border-slate-100 text-[10px] font-bold ">2nd</div>
                                    </div>
                                    <h5 class="font-bold text-slate-800 text-lg mb-1 truncate max-w-[140px] uppercase tracking-widest ">{{ $leaderboardData[1]->name }}</h5>
                                    <div class="text-blue-600 font-bold  tracking-widest text-sm">{{ $leaderboardData[1]->formatted_score }} PTS</div>
                                </div>
                                <div class="w-full h-40 md:h-48 bg-gradient-to-t from-slate-300 to-slate-200 rounded-t-[2rem] shadow-inner flex items-center justify-center relative overflow-hidden">
                                    ...
                                    <div class="text-5xl md:text-7xl font-bold text-white/40  tracking-widest">2</div>
                                </div>
                            @endif
                        </div>
                            ...
                            @if(isset($leaderboardData[0]) && $leaderboardData[0]->total_correct_questions > 0)
                                <div class="text-center mb-8 group relative z-10">
                                    ...
                                    <div class="relative mb-6">
                                        ...
                                        <div class="absolute -bottom-2 -right-2 w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-xl border-2 border-amber-50 text-xs font-bold ">1st</div>
                                    </div>
                                    <h5 class="font-bold text-slate-900 text-xl md:text-2xl mb-1 truncate max-w-[180px]  uppercase tracking-widest">{{ $leaderboardData[0]->name }}</h5>
                                    <div class="text-amber-600 font-bold  tracking-widest text-xl">{{ $leaderboardData[0]->formatted_score }} PTS</div>
                                </div>
                                <div class="w-full h-56 md:h-64 bg-gradient-to-t from-amber-500 to-amber-400 rounded-t-[3rem] shadow-2xl shadow-amber-200/50 flex items-center justify-center relative overflow-hidden">
                                    ...
                                    <div class="text-8xl md:text-9xl font-bold text-white/40  tracking-widest">1</div>
                                </div>
                            @endif
                        </div>
                            ...
                            @if(isset($leaderboardData[2]) && $leaderboardData[2]->total_correct_questions > 0)
                                <div class="text-center mb-8 group">
                                    <div class="relative mb-6">
                                        ...
                                        <div class="absolute -bottom-2 -right-2 w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-lg border-2 border-rose-50 text-[10px] font-bold ">3rd</div>
                                    </div>
                                    <h5 class="font-bold text-slate-800 text-lg mb-1 truncate max-w-[140px] uppercase tracking-widest ">{{ $leaderboardData[2]->name }}</h5>
                                    <div class="text-blue-600 font-bold  tracking-widest text-sm">{{ $leaderboardData[2]->formatted_score }} PTS</div>
                                </div>
                                <div class="w-full h-32 md:h-40 bg-gradient-to-t from-rose-400 to-rose-300 rounded-t-[2rem] shadow-inner flex items-center justify-center relative overflow-hidden">
                                    <div class="absolute inset-x-0 top-0 h-1 bg-white/20"></div>
                                    <div class="text-4xl md:text-6xl font-bold text-white/40  tracking-widest">3</div>
                                </div>
                            @endif
                        </div>
                    </div>
                            ...
                            @foreach($leaderboardData as $data)
                                ...
                                <tr class="group hover:bg-slate-50 transition-all {{ $data->id === auth()->id() ? 'bg-blue-50/50' : '' }}">
                                    <td class="px-6 py-6">
                                        ...
                                        @else
                                            <span class="w-10 text-center block font-bold text-slate-300 ">#{{ $data->rank }}</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-6">
                                        <div class="flex items-center gap-4">
                                            <div class="w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center uppercase font-bold  text-xs shadow-lg shadow-slate-200">
                                                {{ substr($data->name, 0, 1) }}
                                            </div>
                                            <div>
                                                <div class="font-bold text-slate-900  tracking-widest uppercase">{{ $data->name }}</div>
                                    ...
                                    <td class="px-6 py-6">
                                        <div class="w-32 mx-auto">
                                            <div class="flex justify-between items-center mb-1.5 px-0.5">
                                                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ">{{ $data->percentage }}%</span>
                                            </div>
                                            <x-ui.progress-bar :value="$data->percentage" size="xs" :showPercentage="false" variant="{{ $data->badge_color }}" />
                                        </div>
                                    </td>
                                    <td class="px-6 py-6 text-right">
                                        <div class="font-bold text-blue-600  text-xl tracking-widest">
                                            {{ $data->formatted_score }} <span class="text-[10px] text-slate-300 not- uppercase font-bold tracking-widest ml-1">Pts</span>
                                        </div>
                                    </td>
                                </tr>
                                @endif
