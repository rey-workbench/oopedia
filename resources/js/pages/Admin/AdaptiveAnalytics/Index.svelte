...
<App title="Analytics Engine Adaptif - Admin">
    <div class="space-y-8">
                ...
                <div class="flex items-center gap-4">
                    ...
                    {:else if activeTab === 'actions'}
                        <Button
                            ...
                            class="shadow-emerald-900/10 shadow-xl"
                        >
                            TAMBAH AKSI
                        </Button>
                    {/if}
        ...
        <div class="flex items-center gap-2 border-b border-slate-100 px-2">
            {#each tabs as tab}
                <button
                    ...
                    class="relative flex items-center gap-2 px-6 py-4 text-xs font-black tracking-widest transition-all uppercase {activeTab ===
                    tab.id
                        ? 'text-primary-600'
                    ...
                    <div class="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
                        <div class="mb-6 flex items-center justify-between">
                            <h3 class="flex items-center gap-3 text-sm font-black text-slate-800">
                                ...
                                <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-500">{domain.count} Aturan</span>
                            </h3>
                        </div>
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                            {#each domain.rules as rule}
                                <div class="group relative overflow-hidden rounded-2xl border border-slate-50 bg-slate-50/50 p-5 transition-all hover:border-primary-100 hover:bg-white hover:shadow-xl">
                                    <div class="mb-3 flex items-start justify-between">
                                        <div class="flex flex-col gap-1">
                                            <span class="font-mono text-[10px] font-black tracking-widest text-primary-500 uppercase">{rule.rule_code}</span>
                                            <h4 class="text-sm font-bold text-slate-900">{rule.name}</h4>
                                        </div>
                                        <button onclick={() => openEditRule(rule)} class="rounded-xl p-2 text-slate-400 transition-colors hover:bg-primary-50 hover:text-primary-600">
                                            <Pencil size={14} />
                                        </button>
                                    </div>
                                    <div class="flex flex-wrap gap-1.5">
                                        {#each rule.required_facts as fact}
                                            <span class="rounded-lg bg-white px-2 py-1 text-[9px] font-bold text-slate-500 shadow-sm">{fact}</span>
                                        {/each}
                                        <span class="mx-1 text-slate-300">→</span>
                                        <span class="rounded-lg bg-emerald-50 px-2 py-1 text-[9px] font-black text-emerald-600 shadow-sm">{rule.action}</span>
                                    </div>
                                </div>
                            {/each}
                        </div>
        ...
        {:else if activeTab === 'actions'}
            <div class="animate-in fade-in slide-in-from-bottom-4 grid grid-cols-1 gap-6 duration-500 md:grid-cols-2 lg:grid-cols-3">
                {#each props.allActions as action}
                    <div class="group relative overflow-hidden rounded-3xl border border-slate-100 bg-white p-6 transition-all hover:shadow-2xl">
                        <div class="mb-4 flex items-start justify-between">
                            <div class="flex items-center gap-3">
                                <div class="bg-emerald-50 text-emerald-600 flex h-10 w-10 items-center justify-center rounded-2xl">
                                    <Target size={20} />
                                </div>
                                <div class="flex flex-col gap-0.5">
                                    <span class="font-mono text-[10px] font-black tracking-widest text-slate-400 uppercase">{action.code}</span>
                                    <h4 class="text-sm font-black text-slate-900">{action.name}</h4>
                                </div>
                            </div>
                            <div class="flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                                <button onclick={() => openEditAction(action)} class="rounded-xl p-2 text-slate-400 transition-colors hover:bg-blue-50 hover:text-blue-600">
                                    <Settings2 size={16} />
                                </button>
                                <button onclick={() => deleteAction(action.id)} class="rounded-xl p-2 text-slate-400 transition-colors hover:bg-rose-50 hover:text-rose-600">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        </div>
                        <p class="mb-4 text-xs font-medium leading-relaxed text-slate-500 line-clamp-2">
                            {action.description}
                        </p>
                        <div class="space-y-2">
                            <div class="flex items-center justify-between text-[10px] font-black tracking-widest text-slate-400 uppercase">
                                ...
                                <span class="text-emerald-600">{action.instructions?.flow || 'NONE'}</span>
                            </div>
                            <div class="h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
                                <div class="h-full bg-emerald-500" style="width: {action.instructions?.flow ? '100%' : '20%'}"></div>
                            </div>
                        </div>
                    </div>
                {/each}
