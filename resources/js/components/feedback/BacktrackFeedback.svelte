<script lang="ts">
    ...
    import { ArrowRight, ArrowDown, BookOpen } from 'lucide-svelte';
    ...
    let { message, nextAction, recommendation, onContinue }: Props = $props();
</script>
...
<div class="p-8 text-center sm:p-12">
    ...
    <!-- Status Badge -->
    <div
        class="inline-flex items-center gap-2 rounded-2xl border-2 border-b-4 border-rose-200 bg-rose-50 px-5 py-2 font-black tracking-widest text-rose-700 uppercase"
    >
        <ArrowDown size={18} />
        Penyesuaian Alur
    </div>
    ...
    {#if recommendation}
        <div
            id="backtrack-recommendation-card"
            class="mb-10 text-left transition-transform active:scale-[0.98]"
        >
            <div class="rounded-3xl border-2 border-b-6 border-rose-200 bg-rose-50 p-6">
                <div class="flex items-center gap-5">
                    <div
                        class="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border-2 border-b-4 border-rose-300 bg-white text-rose-500"
                    >
                        <BookOpen size={32} />
                    </div>
                    <div>
                        <span
                            class="text-[11px] font-black tracking-[0.2em] text-rose-400 uppercase"
                            >Materi Remedial</span
                        >
    ...
    <p class="mt-8 text-[11px] font-black tracking-[0.3em] text-slate-300 uppercase">
        Sistem Adaptif • Membantumu Berkembang
    </p>
</div>
