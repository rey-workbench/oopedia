<script>
    ...
    export let aspects = [];
    export let questionnaireAspects = [];
    ...
    <div class="p-12 space-y-12">
        {#if $form.errors && Object.keys($form.errors).length > 0}
            <Alert variant="danger" dismissible={true}>
                Ada {Object.keys($form.errors).length} aspek yang belum Anda evaluasi
                atau tidak valid. Silakan tinjau kembali input Anda.
            </Alert>
        {/if}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-3">
                        <label
                            for="comments"
                            class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-4"
                            >Komentar Subjektif <span class="text-rose-500"
                                >*</span
                            ></label
                        >
                        <textarea
