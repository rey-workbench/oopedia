<script>
    ...
    /**
     * Adaptive Indicator - Debug Mode Only
     *
     * To enable this debug panel, add to your .env file:
     * VITE_ADAPTIVE_DEBUG=true
     *
    ...
    // Group facts by category for better visualization
    $: factCategories = {
        score: facts.filter((f) => ["G01", "G02", "G03", "G04"].includes(f)),
        time: facts.filter((f) => ["G05", "G06"].includes(f)),
        style: facts.filter((f) => ["G07", "G08"].includes(f)),
        error: facts.filter((f) => ["G09", "G10"].includes(f)),
        hint: facts.filter((f) => ["G11", "G12"].includes(f)),
        module: facts.filter((f) =>
    ...
    >
        ...
        >
            <div class="flex items-center gap-4">
                <div
                    class={`w-8 h-8 rounded-lg bg-white/20 backdrop-blur-md flex items-center justify-center ${isProcessing ? "animate-pulse" : ""}`}
                >
                ...
                <div class="text-left">
                    ...
                    >
                        ...
                        {#if isCollapsed}
                            ...
                            >
                                {facts.length} Facts • {triggeredRule
                                    ? "Rule Active"
                                    : "No Rule"}
                            </Badge>
                        {/if}
            ...
            <div class="flex items-center gap-4">
                ...
                {/if}
                <svelte:component
                    this={isCollapsed ? ChevronUp : ChevronDown}
                    size={20}
                />
            </div>
        </button>
                ...
                <div class="max-w-7xl mx-auto">
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <span>Facts Gathered ({facts.length})</span>
                                <span
                            ...
                            >
                                {#each Object.entries(factCategories) as [category, categoryFacts]}
