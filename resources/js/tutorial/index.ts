...
export function getTourIdFromUrl(url: string, isAdminRole: boolean): string {
    ...
    if (isAdminRole) {
        ...
        else if (url.includes('/admin/questions')) {
            if (url.includes('/create') || url.includes('/edit')) tourId = 'admin_question_editor';
            else tourId = 'admin_questions';
        }
    } else {
        if (url.includes('/mahasiswa/dashboard/in-progress'))
            ...
            // Check if it's a specific material's quiz session or the global quiz index
            const questionParts = url.split('/questions');
            const materialsParts = questionParts[0]?.split('/materials/');
            
            // If there's an ID between /materials/ and /questions, it's a session
            // Example session: /mahasiswa/materials/01knq9.../questions
            // Example index: /mahasiswa/materials/questions
            if (materialsParts?.[1] && materialsParts[1].length > 0) {
                if (url.includes('/questions/levels')) tourId = 'mahasiswa_quiz_levels';
                else if (url.includes('/questions/review')) tourId = 'mahasiswa_quiz_review';
                else tourId = 'mahasiswa_quiz_session';
            } else {
