...
| `bootstrap/app.php` | Laravel application bootstrapper (middleware, exceptions) |

## Subdirectories
| Directory | Purpose |
|-----------|---------|
| `app/` | Backend Core (Interface-first) (see `app/AGENTS.md`) |
| `bootstrap/` | Laravel bootstrapping and cache |
| `config/` | Application configuration files (see `config/AGENTS.md`) |
| `database/` | Migrations, seeders, and factories (see `database/AGENTS.md`) |
| `lang/` | Language files |
| `public/` | Publicly accessible files and Vite build output |
...
### Common Patterns
- Bypassing service/repository interfaces is strictly forbidden.
- RTK proxy required for all shell commands (`rtk php artisan test --compact`, `rtk vendor/bin/pint --dirty`, `rtk pnpm run format:check`).
