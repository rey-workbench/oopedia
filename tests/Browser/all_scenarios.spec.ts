...
async function getFirstMaterialId(page: Page): Promise<string> {
    ...
    const materialLink = page.locator('a[href*="/mahasiswa/materials/"]:not([href*="questions"])').first();
...
test.describe('Dashboard Mahasiswa', () => {
    ...
    test.describe('Profil', () => {
        ...
        test('Scenario 38: Ganti Kata Sandi Profil', async ({ page }) => {
            ...
            if (await passwordField.count() > 0) {
                ...
                const confirmPassField = page.locator('#password_confirmation, input[name="password_confirmation"]');
    ...
    test.describe('Keamanan Akses Halaman (Security)', () => {
        ...
        test('Scenario 93: Mahasiswa Tidak Bisa Akses Halaman Admin', async ({ page }) => {
            ...
            const isBlocked = status === 403 || !url.includes('admin/dashboard') || (await page.locator('text=403').count()) > 0 || (await page.locator('text=Forbidden').count()) > 0;
